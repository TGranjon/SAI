...
{
    printf("Arbre4\n");
    printf("(%f, %f),(%f, %f)\n",A[0].x,A[0].z,A[2].x,A[2].z);
    if(inclus(A)==FALSE)
        return ConsArbre(VIDE,A[0],A[2],ArbreVide(),ArbreVide(),ArbreVide(),ArbreVide());
    int res = contenu(A);
    if(res==3)
        return ConsArbre(PLEIN,A[0],A[2],ArbreVide(),ArbreVide(),ArbreVide(),ArbreVide());
    if(res==1)
        return ConsArbre(VIDE,A[0],A[2],ArbreVide(),ArbreVide(),ArbreVide(),ArbreVide());
    if(res==2)
    {
        ...
        B4[3].z=A[0].z;
        printf("Inter\n");
        return ConsArbre(INTER,A[0],A[2],Arbre4(B1),Arbre4(B2),Arbre4(B3),Arbre4(B4));
    }
    return ArbreVide(); // Cas d'erreur
}

void visible(arbre ar)
{
    if(ar->racine==PLEIN)
    {
        P[Pnum]=ar->A;
